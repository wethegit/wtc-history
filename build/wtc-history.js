...
function () {
  ...
  _createClass(History, null, [{
    ...
    value: function push(URL) {
      ...
      var parsedURL = '';
      this.currentState = stateObj; // First try to fix the URL
      ...
      stateObj = Object.assign({}, stateObj, {
        historyStateURL: parsedURL,
        historyStateTitle: title
      });
      this.emitEvent('WTCHistory-push', stateObj); // If we have API support, push the state to the history object
    ...
    value: function _fixURL(URL) {
      ...
      var URLRegex = RegExp("^([^:]+://([^/]+))?(".concat(this.documentRoot, ")?(/)?([^\\#\\?]*)(\\?[^\\#]*)?(\\#\\!?.+)?"));
      ...
      var _URLRegex$exec = URLRegex.exec(URL),
          ...
          hashbang = _URLRegex$exec2[7]; // If we're observing the TLDN restraint and the provided URL doesn't match
      ...
      if (typeof root === 'string' && root === '/' || typeof documentRoot === 'string' && documentRoot === this.documentRoot) {
        if (includeDocRoot && this.documentRoot !== '/') {
          rtnURL = "".concat(this.documentRoot, "/").concat(path);
        } else {
          rtnURL = "/".concat(path);
        } // Else if path has resulted in an empty string, assume the path is the root

      } else if (path === '') {
        rtnURL = '/'; // Otherwise, just pass the path completely.
      } else {
        rtnURL = path;
    ...
    set: function set() {
      ...
      var docrootRegex = /^([^:]+:\/\/([^\/]+))?\/?(.*(?=\/))?/; // pass the docroot and hostname
      ...
      var _docrootRegex$exec = docrootRegex.exec(documentRoot),
          ...
          docroot = _docrootRegex$exec2[3]; // Error check
      ...
      if (docroot) {
        this._documentRoot = "/".concat(docroot);
      }
    },
    get: function get() {
